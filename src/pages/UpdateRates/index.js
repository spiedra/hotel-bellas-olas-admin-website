...
const UpdateRates = () => {
  ...
  const setEditValue = (rateId) => {
    setCurrentEditRate(roomRates.find((rr) => { return rr.id === rateId }))
  ...
  return (
    <>
        ...
        {
            roomRates
              ? <CustomizedTable action={true} onEdit={setEditValue} withImage={'img'} onDelete={(onConfirmDelete)} columns={columns} rows={roomRates || []} />
              : <LoaderSpinner/>
        }
        <Modal
        ...
        <Modal
            ...
            modalBody={
                <>Se eliminará la tarifa #{currentDeleteRate}</>
            }
            modalActions={
                <>
                    <Button
