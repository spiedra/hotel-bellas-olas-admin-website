...
const UpdateAboutUs = () => {
  const [aboutUsInfo, setAboutUsInfo] = useState({ aboutUsText: '', imgList: [] })
  const [isInsertModalOpen, setIsInsertModalOpen] = useState(false)
  const [currentEditImage, setCurrentEditFeature] = useState({ id: '', image: '' })
  ...
  const fetchAboutUsInfo = () => {
    getAboutUsInfo().then(response => {
      setAboutUsInfo({ aboutUsText: response.aboutUsText, imgList: response.imgList })
    })
  }
  useEffect(() => {
    fetchAboutUsInfo()
  ...
  const onSaveAboutUsText = async (values) => {
    ...
    formData.append('AboutUsText', aboutUsInfo.aboutUsText)
  ...
  const setEditValue = (imageId) => {
    ...
    setCurrentEditFeature(aboutUsInfo.imgList.filter((img) => { return img.id === imageId })[0])
  }
  ...
  const onSaveChangeImage = async () => {
    if (fileInput.current.files[0]) {
      ...
      setStateModal({ msg: 'Debe seleccionar una imagen primero', isOpen: true })
    }
    fetchAboutUsInfo()
  }
