...
const columns = [
  ...
  { id: 'adLink', label: 'Link', maxWidth: 400 },
  { id: 'info', label: 'Info', maxWidth: 400 },
  {
    id: 'img',
    label: 'Imagen',
    minWidth: 170,
...
const Advertising = () => {
  ...
  const [currentEditAdvertising, setCurrentEditAdvertising] = useState({ id: '', adLink: '', info: '', img: '' })
  ...
  const setEditValue = (featureId) => {
    ...
    setCurrentEditAdvertising(advertisingList.filter((f) => { return f.id === featureId })[0])
  }
  ...
  return (
    <Box sx={advertisingStyles.mainContainer}>
      ...
      <AddButton onAdd={() => setIsInsertModalOpen(true)}/>
        {
            advertisingList
              ? <CustomizedTable action={true} onEdit={setEditValue} withImage={'img'} onDelete={(onConfirmDelete)} columns={columns} rows={advertisingList || []} />
              : <LoaderSpinner/>
        }
        <Modal
        ...
        <Modal
            ...
            modalBody={
                <>Se eliminará la publicidad #{currentDeleteAdvertising}</>
            }
            modalActions={
                <>
                    <Button
